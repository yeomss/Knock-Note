			...
			<div class="note-favorite">
              ...
              ></i> -->
						...
						<div v-show="note.img_path" class="note-image-wrap">
							...
							/>
							<div
								v-show="note.img_comment_modal"
								class="imageCommentModal"
							>
								{{ note.predicted }}
							</div>
						</div>
						...
						<span v-if="note.category != 'To-do List'">
							<span class="textform-B" @click="setBold(index)"
								>B</span
							>
							<span class="textform-U" @click="setUnderbar(index)"
								>U</span
							>
							<span
								class="textform-I"
								@click="setInclination(index)"
								>I</span
							>
						</span>
						...
						<div
							class="note-colorform"
							v-show="notes[index].is_show"
						>
							<ul>
								<li
									class="color1"
									@click="setNoteColor(index, colors[0])"
								></li>
								<li
									class="color2"
									@click="setNoteColor(index, colors[1])"
								></li>
								<li
									class="color3"
									@click="setNoteColor(index, colors[2])"
								></li>
								<li
									class="color4"
									@click="setNoteColor(index, colors[3])"
								></li>
								<li
									class="color5"
									@click="setNoteColor(index, colors[4])"
								></li>
							</ul>
						</div>
						...
						<div class="imageInputBox">
							...
							<button
								class="imageInputBtn"
								v-on:click="setFileExploer(index)"
							>
								이미지 업로드
							</button>
							<span class="note-face">
								<i
									...
									@click="detectEmotion(index)"
								></i>
							</span>
							<span class="emotionModal"
								>I FEEL LIKE #{{ note.emotion }}</span
							>
						</div>
					</div>
				</tr>
				...
				<div v-if="note.lock != true">
					...
					<span v-if="note.category != 'To-do List'">
						...
						<span class="textform-U" @click="setUnderbar(index)"
							>U</span
						>
						<span class="textform-I" @click="setInclination(index)"
							>I</span
						>
					</span>
					<span class="category-form">
					...
					<div class="imageInputBox">
						...
						<span v-else>
							<button
								class="lockBtn_TodoList"
								@click="modalLock(index)"
							>
								노트 잠금
							</button>
						</span>
...
export default {
	...
	methods: {
		...
		detectEmotion: async function (index) {
			...
			await axios
				.post("http://127.0.0.1:3000/face", {
					//fileUrl: file,
					fileUrl:
						"D:/GitKumoh/Documents/osp20-hello/src/assets/" +
						filename,
					//fileUrl: file,
				})
				.then((res) => {
		...
		translateNote: async function (index) {
			this.notes[index].translate_modal =
				!this.notes[index].translate_modal;
	...
	async mounted() {
		...
		if (localStorage.getItem("notes")) {
			...
			let baseURL =
				"https://teachablemachine.withgoogle.com/models/OsUYBFECF/";
